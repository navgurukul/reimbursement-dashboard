...
// import { expenses,logExpenseStatusChange } from "@/lib/db";
...
//       try {
//   await logExpenseStatusChange(
//     expenseId,
//     currentUserId,
//     expense.status, // Old status
//     "approved", // New status
//     `Approved with custom amount: ${approvedAmount}` // Add note about custom amount
//   );
// } catch (logError) {
...
//         currentUserId,
//         expense.status, // Old status
//         updateData.status, // New status
//         null // No notes for standard approval
//       );
//     } catch (logError) {
//       console.error("Error logging status change:", logError);
//       // Don't block approval if logging fails
//     }
...
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { ArrowLeft, Check, X, FileText, AlertCircle, Edit, Clock } from "lucide-react";
import { Spinner } from "@/components/ui/spinner";
import { PolicyAlert } from "@/components/policy-alert";
import { Input } from "@/components/ui/input";
import supabase from "@/lib/supabase";
...
export default function ViewExpensePage() {
  ...
  const getExpenseTitle = () => {
    ...
    return expense.creator?.full_name ? `Expense Details ${expense.creator.full_name}` : "Expense Details";
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* New layout with two cards side by side */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Left column - Expense Details Card */}
        <Card>
          <CardHeader>
            <CardTitle>{getExpenseTitle()}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
            ...
            {expense.custom_fields &&
              Object.keys(expense.custom_fields).length > 0 && (
                <div className="grid grid-cols-2 gap-4 mt-4">
                  {Object.entries(expense.custom_fields).map(([key, value]) => {
                    // Skip certain fields that might be used for vouchers
                    if (key === 'isVoucher' || key === 'yourName' || key === 'voucherDate' || 
                        key === 'voucherAmount' || key === 'purpose' || key === 'voucherCreditPerson') {
                      return null;
                    }
                    return (
                      <div key={key}>
